  ...
  let state = {
    ...
    rewards: [
      ...
      { id: 6, title: '1 partida de bolos', cost: 300, icon: 'Bowling' }
    ],
    redemptions: [],
    history: []
  };
  ...
  // --- PANEL DE GESTIÓN PARENTAL Y EDICIÓN ---
  function renderManagerPanel() {
    if (!isManagerUnlocked) return;

    const redemptionsList = document.getElementById('managerRedemptionsList');
    if (redemptionsList) {
      if (state.redemptions.length === 0) {
        redemptionsList.innerHTML = `<p class="text-xs text-zinc-500 py-2">No hay canjes pendientes de entregar.</p>`;
      } else {
        redemptionsList.innerHTML = state.redemptions.map(r => `
          <div class="bg-zinc-950 p-3 rounded-2xl border border-zinc-800 flex items-center justify-between gap-2">
            <div class="flex items-center gap-3">
              <span class="text-2xl">${r.rewardIcon}</span>
              <div>
                <p class="text-xs font-black text-white">${r.rewardTitle}</p>
                <p class="text-[10px] text-zinc-400">Por <span class="text-amber-300 font-bold">${r.userName}</span> • ${r.date}</p>
              </div>
            </div>
            <button onclick="fulfillRedemption(${r.id})" class="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-[10px] rounded-xl active:scale-95 transition">
              Entregado ✅
            </button>
          </div>
        `).join('');
    ...
    const actionsList = document.getElementById('managerActionsList');
    if (actionsList) {
      actionsList.innerHTML = state.actions.map(a => `
        <div class="bg-zinc-950 p-3 rounded-2xl border border-zinc-800 flex items-center justify-between gap-2">
          <div class="flex items-center gap-2 truncate">
            <span>${a.icon || '⭐'}</span>
            <span class="text-xs font-bold text-zinc-200 truncate">${a.title}</span>
            <span class="text-[10px] font-black ${a.type==='positive'?'text-emerald-400':'text-red-400'}">(${a.points > 0 ? '+' : ''}${a.points} pts)</span>
          </div>
          <button onclick="deleteAction(${a.id})" class="text-red-400 hover:text-red-300 text-xs font-black p-1">❌</button>
        </div>
      `).join('');
    }

    const rewardsList = document.getElementById('managerRewardsList');
    if (rewardsList) {
      rewardsList.innerHTML = state.rewards.map(r => `
        <div class="bg-zinc-950 p-3 rounded-2xl border border-zinc-800 flex items-center justify-between gap-2">
          <div class="flex items-center gap-2 truncate">
            <span>${r.icon || '🎁'}</span>
            <span class="text-xs font-bold text-zinc-200 truncate">${r.title}</span>
            <span class="text-[10px] font-black text-amber-400">(${r.cost} pts)</span>
          </div>
          <button onclick="deleteReward(${r.id})" class="text-red-400 hover:text-red-300 text-xs font-black p-1">❌</button>
        </div>
      `).join('');
    }

    const doubleXpBtn = document.getElementById('btnToggleDoubleXp');
    if (doubleXpBtn) {
      doubleXpBtn.className = state.doubleXpActive
        ? "w-full py-3 bg-red-600 hover:bg-red-500 text-white font-black text-xs rounded-xl shadow-lg transition"
        : "w-full py-3 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-black text-xs rounded-xl shadow-lg transition";
      doubleXpBtn.innerText = state.doubleXpActive ? "Desactivar Doble XP ❌" : "Activar Evento Doble XP (2x) 🔥";
    }
  }
  ...
  async function addNewAction() {
    ...
    if (!title) return;
    const ptsStr = prompt("Puntos (positivos para premio, negativos para castigo):");
    const points = parseInt(ptsStr);
    if (isNaN(points)) return alert("Cantidad invalida.");
    const icon = prompt("Icono Emoji (ej: 🧹, 📖):") || "⭐";
    const type = points >= 0 ? "positive" : "negative";

    state.actions.push({ id: Date.now(), title, points, type, icon });
    await saveLocalStorage();
    renderApp();
  }
  ...
  async function addNewReward() {
    const title = prompt("Título del premio:");
    if (!title) return;
    const costStr = prompt("Coste en puntos:");
    const cost = parseInt(costStr);
    if (isNaN(cost) || cost <= 0) return alert("Coste inválido.");
    const icon = prompt("Icono Emoji (ej: 🎮, 🍦):") || "🎁";
  ...
  async function resetUserPointsPrompt() {
    const choice = prompt("Escribe '1' para reiniciar a Joan o '2' para Martina:");
    let user = null;
    if (choice === '1') user = state.users['joan'];
    else if (choice === '2') user = state.users['martina'];
    else return;

    if (user && confirm(`¿Seguro que deseas poner a 0 los puntos de ${user.name}?`)) {
      user.points = 0;
      state.history.unshift(`⚙️ Puntos de ${user.name} reiniciados a 0 por el administrador.`);
      await saveLocalStorage();
      renderApp();
    }
  }

  async function updateFamilyGoalPrompt() {
    const title = prompt("Nombre de la Meta Familiar Co-op:", state.familyGoal?.title || "👾 El Dragón del Desorden");
    if (!title) return;
    const targetStr = prompt("Puntos totales para superar la meta:", state.familyGoal?.targetPoints || 500);
    const targetPoints = parseInt(targetStr);
    if (isNaN(targetPoints) || targetPoints <= 0) return alert("Cantidad de puntos inválida.");

    state.familyGoal = { title, targetPoints };
    await saveLocalStorage();
    renderApp();
  }
  ...
  function exportData() {
    ...
    downloadAnchor.setAttribute("download", `family_points_backup_${new Date().toISOString().slice(0,10)}.json`);
